...
export default function MediaCarousel({
  ...
  const imgLoader = ({ src }: { src: string }) => {
    return `${GOOGLE_DRIVE_EXPORT}${src}?timestamp=${Date.now()}`;
  };

  const renderImgList = (): Array<React.ReactElement> | undefined => {
    return imageData?.files && imageData?.files.map((item: ImageProps, idx: number) => {
      return (
          ...
          <Grid css={styles.headline} md={4} mdOffset={1}>
            <Typography variant="h5" component="p">
              ...
              <Typography component="span" variant="h5" color="primary.light">
                unparalleled support to every client,
              </Typography>{" "}
                ensuring a seamless journey as they discover and {" "}
              <Typography component="span" variant="h5" color="primary.light">
                secure the home   
              </Typography>
                &nbsp;that perfectly aligns with their dreams and aspirations.
            </Typography>
          </Grid>
          {imageData && (
